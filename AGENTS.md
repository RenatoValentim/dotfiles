...
- `config/`: primary dotfiles, intended to be linked into `~/.config` (XDG).
  ...
  - `config/wezterm/`: WezTerm config (Lua).
...
- `.github/workflows/ci.yml`: CI runs syntax/format checks.

## Build, Test, and Development Commands

- `./scripts/check.sh`: runs local checks (Fish syntax and StyLua formatting checks).
...
- Lua (Neovim/WezTerm): format with StyLua. Repo settings: 2-space indent, 120 columns (`config/lazyvim/stylua.toml`).
...
There are no unit tests; treat `./scripts/check.sh` and CI as the quality gate. Add new checks to `scripts/check.sh` when introducing new languages/tools.
