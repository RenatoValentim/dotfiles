...
map("n", "<leader>w", ":wall<Return>", key_options({ desc = "Save All" }))
map("n", "<C-q>", ":q<Return>", key_options({ desc = "Quit Current File" }))
map("n", "<leader>mo", ":only<Return>", key_options({ desc = "Keeps only the current window" }))
...
map("n", "g[x", "<Plug>(git-conflict-prev-conflict)")
map("n", "g]x", "<Plug>(git-conflict-next-conflict)")

map("n", "<leader>m", "", key_options({ desc = "My Personal Keys" }))

map("n", "<leader>mh", ":Hardtime toggle<Return>", key_options({ desc = "Hardtime toggle" }))
map("n", "<leader>mdb", ":Dbee<Return>", key_options({ desc = "Dbee SGBD" }))
map("n", "<leader>mp", ":Store<CR>", { desc = "Plugin Manager" })
